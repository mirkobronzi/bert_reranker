...
if mode == 'visualize':
    ...
    visited = [] 
    plt.scatter(emb[:N_in, 0], emb[:N_in, 1], c='r', marker='o', label='covid_quebec_faq')
    if dataset_ood == 'samasource_ood': 
        ...
        for i, (an) in enumerate(np.unique(ood_questions['annotations'])):
            ...
            plt.scatter(emb_ood[inds, 0], emb_ood[inds, 1], c=colors[i], marker=markers[i], label=an)
    else:
    ...
    with open('results_samasource.txt', 'w') as fl:
       for i, (qst, an) in enumerate(zip(ood_questions['questions'], ood_questions['annotations'])):
            fl.write(qst + '\n')
