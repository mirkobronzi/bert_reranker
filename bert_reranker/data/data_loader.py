...
def encode_sentence(sentence, max_length, tokenizer):
    ...
    return {
        "ids": input_question["input_ids"].squeeze(0),
    ...
    for source, passages in source2passages.items():
        for passage in passages:
            if is_in_distribution(passage):
                ...
                encoded_passage = encode_sentence(
                    passage_text, max_passage_length, tokenizer
                )
...
class ReRankerDataset(Dataset):
    def __init__(self, json_file, max_example_len, max_passage_len, tokenizer):
        ...
        source2passages, pid2passage, pid2index = get_passages_by_source(json_data)
        ...
        logger.info(
            "loaded passages from file {} - found {} sources".format(
                json_file, len(source2id)
            )
        )
        for source in source2id.keys():
            logger.info(
                'source "{}": found {} in-distribution and {} out-of-distribution'.format(
    ...
    def __getitem__(self, idx):
        ...
        encoded_question = encode_sentence(
            question, self.max_example_len, self.tokenizer
        )
