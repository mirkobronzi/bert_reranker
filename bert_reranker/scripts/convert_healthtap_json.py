...
def main():
    ...
    parser.add_argument(
        "--input", help="input csv file (with healthtap)", required=True
    )
    parser.add_argument(
    ...
    with open(
        os.path.join(args.output, "healthtap_train.json"), "w", encoding="utf-8"
    ) as ostream:
        json.dump(qa_pairs_train, ostream, indent=4, ensure_ascii=False)

    with open(
        os.path.join(args.output, "healthtap_dev.json"), "w", encoding="utf-8"
    ) as ostream:
        json.dump(qa_pairs_dev, ostream, indent=4, ensure_ascii=False)

    with open(
        os.path.join(args.output, "healthtap_test.json"), "w", encoding="utf-8"
    ) as ostream:
