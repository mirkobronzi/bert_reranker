...
def evaluate_tokenizer_cutoff(file, tokenizer, max_question_length, max_answer_length):
    ...
    evaluate_tokenizer_cutoff_from_json(
        qa_pairs, tokenizer, max_question_length, max_answer_length
    )


def evaluate_tokenizer_cutoff_from_json(
    qa_pairs, tokenizer, max_question_length, max_answer_length
):
    ...
    cutoff_results_questions = count_cutoff_sentences(
        all_questions, tokenizer, max_question_length
    )
    cutoff_results_answers = count_cutoff_sentences(
        all_answers, tokenizer, max_answer_length
    )

    logger.info("Max length used for questions: {}".format(max_question_length))
    logger.info(
        "Number of questions cutoff by tokenizer: {} / {}, ({:3.2f} %)\n".format(
    ...
    logger.info(
        "Number of answers cutoff by tokenizer: {} / {}, ({:3.2f} %)\n".format(
...
def evaluate_tokenizer_cutoff_from_txt(sentences, tokenizer, max_sentence_length):
    ...
    # Analyze how much is being cutoff
    cutoff_results_questions = count_cutoff_sentences(
        sentences, tokenizer, max_sentence_length
    )
...
def main():
    ...
    parser.add_argument(
        "--input", help="data file containing questions and answers", required=True
    )
    parser.add_argument(
        "--tokenizer-name", help="name of the tokenizer to use", required=True
    )
...
def analyze_txt(args, tokenizer):
    ...
    for i in range(5):
        ...
        logger.info(
            'question {} "{}" => "{}"'.format(i, sentence, tokenizer.tokenize(sentence))
        )
    max_lengths = [
        10,
        20,
...
def analyze_json(args, tokenizer):
    ...
    for i in range(5):
        ...
        logger.info(
            'question {} "{}" => "{}"'.format(i, question, tokenizer.tokenize(question))
        )
        logger.info(
            'candidate {} "{}" => "{}"\n'.format(
