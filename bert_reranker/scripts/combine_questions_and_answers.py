...
def main():
    ...
    parser.add_argument(
        "--input-questions",
        help="files with one question per line",
        required=True,
        nargs="+",
    )
    parser.add_argument(
        "--input-candidates", help="file with one candidate per line", required=True
    )
    ...
    for input_question in args.input_questions:
        _generate_json(
            input_question,
            os.path.join(args.output, ntpath.basename(input_question)) + ".json",
            candidates,
        )
...
def _generate_json(input_file, output_file, candidates):
    ...
    logger.info(
        "file {}: used {} candidates and {} questions".format(
