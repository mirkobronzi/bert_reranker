...
class Retriever(nn.Module):
    ...
    def compute_embeddings(self, question, passages):
        ...
        for i in range(num_document):
            h_paragraph = checkpoint(
                self.bert_paragraph_encoder,
                passages[i]["ids"],
                passages[i]["am"],
                passages[i]["tt"],
                self.dummy_tensor,
    ...
    def predict(self, question, enc_passages):
        ...
        with torch.no_grad():
            ...
            enc_question = encode_sentence(
                question, self.max_question_len, self.tokenizer
            )
            ...
            relevance_scores = self.compute_score(
                question=enc_question, passages=enc_passages
            )
...
class FeedForwardRetriever(Retriever):
    def __init__(
        ...
        ffw_layers = get_ffw_layers(
            previous_hidden_size * 2,
            model_hyper_params["dropout"],
            model_hyper_params["retriever_layer_sizes"] + [1],
            False,
        )
    ...
    def forward(
        ...
        concatenated_embs = torch.cat(
            (q_emb.unsqueeze(1).repeat(1, n_paragraph, 1), p_embs), dim=2
        )
