...
def load_model(hyper_params, tokenizer, debug):
    ...
    if hyper_params["model"]["name"] == "bert_encoder":
        ...
        if (
            bert_question_encoder.post_pooling_last_hidden_size
        ...
        model = EmbeddingRetriever(
            cnn_question_encoder,
            cnn_paragraph_encoder,
            tokenizer,
            hyper_params["max_question_len"],
            hyper_params["max_paragraph_len"],
            debug,
        ...
        raise ValueError(
            "model name {} not supported".format(hyper_params["model"]["name"])
        )
    return model
...
def _create_encoders(encoder, hyper_params):
    if hyper_params["model"]["single_encoder"]:
        ...
        bert_question_encoder = encoder(hyper_params, bert_model=None, name="question")
        bert_paragraph_encoder = encoder(
            hyper_params, bert_model=None, name="paragraph"
        )
